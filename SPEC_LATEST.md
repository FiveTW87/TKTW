...
### 4.2 Product Decisions ที่ยืนยันแล้ว

| ID | เรื่อง | ข้อสรุป |
|---|---|---|
| OD-001 | “ส่งไปส่อง” | **Closed — ไม่ใช่ Bug** ทดสอบใหม่แล้วทำงานถูกต้อง เป็นความเข้าใจคลาดเคลื่อน จึงไม่ต้องแก้ Logic, ไม่ต้องสร้าง TODO และไม่ต้องเพิ่ม Regression Test สำหรับประเด็นนี้ |
| OD-002 | เฟสจั่ว | ผู้เล่นกดปุ่มจั่วเอง หากหมดเวลา Server/Engine จั่วให้อัตโนมัติ โดยใช้ Default Decision ที่ปลอดภัย |
| OD-003 | Role และการเลือก General | แสดง Role ของตนเองก่อนเลือก General ชั่วครู่ จากนั้นเจ้าเมืองเลือกก่อนและเปิด General ทันที ส่วนผู้เล่นอื่นเลือกแบบส่วนตัวตามลำดับ และเปิด General พร้อมกันเมื่อทุกคนเลือกเสร็จ |
| OD-004 | หมด Grace Period กลาง Match | ตัวละครถือว่าเสียชีวิต เปิด Role และล้าง Zone ตาม Death Flow โดยไม่มีผู้สังหารและไม่มีรางวัลสังหาร หากเจ้าเมืองออกหรือหมด Grace Period ให้จบเกมแบบ **ไม่มีผู้ชนะ** |
| OD-005 | Seat ของ Match | สุ่ม Seat ใหม่ทุก Match รวมทั้ง Match แรกและ Rematch ห้ามผูกเจ้าเมืองกับ Seat 1 |
| OD-006 | Bug สกิลโจโฉ | สกิลเก็บการ์ดที่สร้างความเสียหายต้อง Trigger เฉพาะเมื่อ **โจโฉเจ้าของสกิลได้รับความเสียหายเอง** ห้าม Trigger เมื่อผู้เล่นก๊กเดียวกันได้รับความเสียหาย |
| OD-007 | Result Role Reveal | หน้าสรุปเปิด Role ของผู้เล่นทุกคน แสดงผู้ชนะ และสถิติเด่น เช่น สังหารมากที่สุดและได้รับความเสียหายมากที่สุด |
| OD-008 | Grace Period | ใช้ช่วง 30–60 วินาที โดยค่าเริ่มต้น **45 วินาที** และให้ Server config ได้ |

รายการ Product Decision ชุดนี้ไม่มี Blocker คงเหลือแล้ว

> **สำคัญ:** คำว่า “ยืนยันแล้ว” หมายถึง Requirement ถูกตัดสินแล้วเท่านั้น ไม่ได้หมายความว่า Implementation เสร็จ งานใน `TODO.md` ต้องคงเป็น `[ ]` จนกว่าโค้ด, Test และ Review จะผ่าน
...
#### ENG-004 — การจั่วและสกิลในเฟสจั่ว

Flow ที่ตกลงแล้ว:

1. เมื่อเข้าสู่เฟสจั่ว Server สร้าง Draw Decision พร้อม `expiresAt`
2. ผู้เล่นกดปุ่ม `จั่วการ์ด`
3. Engine คำนวณจำนวนการ์ดสุดท้ายและจั่วทั้งหมดใน Transaction เดียว
4. หากผู้เล่นไม่กดภายในเวลา Server ส่ง Default Answer เพื่อจั่วให้อัตโนมัติ
5. Double click, retry หรือ reconnect ห้ามทำให้จั่วซ้ำ

สกิลในเฟสจั่วตามชุดนายพลปัจจุบัน:

| นายพล | สกิล | ประเภท | Interaction |
|---|---|---|---|
| เตียวเลี้ยว | แปดร้อยทลายค่าย | ทางเลือกแทนการจั่ว | ต้องถามก่อนจั่วว่าจะใช้สกิลหรือจั่วปกติ หากใช้ให้เลือกผู้เล่นที่ชิงการ์ดตามกฎ |
| เคาทู | เปลือยเกราะท้าศึก | ทางเลือกจั่วน้อยลง | ต้องถามก่อนจั่วว่าจะใช้สกิลหรือไม่ หากใช้ให้ลดจำนวนจั่ว 1 และเปิดสถานะเพิ่มความเสียหายจนจบเทิร์น |
| จิวยี่ | ปรีชาเจียงตง | สกิลบังคับจั่วเพิ่ม | ไม่ต้องถาม เมื่อกดจั่วให้จั่วรวม 3 ใบทันทีและแสดง Banner/Toast ว่าสกิลทำงาน |

กฎ Default เมื่อหมดเวลา:

- ไม่เลือกใช้สกิลทางเลือกของเตียวเลี้ยวหรือเคาทู
- ใช้การจั่วปกติพร้อม Modifier แบบบังคับ เช่น ปรีชาเจียงตง
- แสดงผลการทำงานของสกิลบังคับให้ผู้เล่นเห็น
- Draw Decision ต้อง resolve เพียงครั้งเดียว

Acceptance:

- ป้องกัน double click
- ป้องกัน retry ทำให้จั่วซ้ำ
- Skill ที่เพิ่ม ลด หรือแทนการจั่วทำงานถูกต้อง
- จำนวนที่จั่วคำนวณก่อน mutate
- Timeout ใช้ Default Answer ข้างต้น
- Rejoin ก่อนตัดสินใจเห็น Decision เดิมและ Deadline เดิม
- Rejoin หลังจั่วเห็น State ล่าสุดและไม่มีปุ่มจั่วซ้ำ
...
#### ENG-006 — สกิลโจโฉ Trigger ผิดเมื่อพันธมิตรได้รับความเสียหาย

ปัญหาที่ Reproduce ได้:

```text
Given:
- Player A ใช้โจโฉและเป็นเจ้าของสกิล “พลิกภัยเป็นกล”
- Player B เป็นนายพลก๊กเดียวกับโจโฉ
- Player B ได้รับความเสียหายจากการ์ด

Current incorrect behavior:
- สกิลของโจโฉ Trigger และโจโฉสามารถเก็บการ์ดที่ทำความเสียหายแก่ Player B

Expected behavior:
- สกิลต้องไม่ Trigger
- สกิล Trigger ได้เฉพาะเมื่อ damagedPlayerId === skillOwnerPlayerId
```

Acceptance:

- โจโฉได้รับความเสียหายเอง → สามารถเก็บ Source Card ตามกฎ
- ผู้เล่นก๊กเดียวกันได้รับความเสียหาย → สกิลไม่ Trigger
- ผู้เล่นต่างก๊กได้รับความเสียหาย → สกิลไม่ Trigger
- Faction และ Lord Skill ไม่มีผลต่อ Owner scope ของสกิลนี้
- โจโฉตายแล้วสกิลไม่ Trigger
- Damage หลายครั้งสร้าง Trigger เฉพาะ Event ที่โจโฉเป็นเป้าหมาย
- ตรวจ Trigger registry ว่าไม่ได้ลงทะเบียนสกิลแบบ faction-wide
...
### 5.4 สถานะ Gate — ✅ ผ่าน (Phase 1 เสร็จ)

- Focused tests ต่อทุก ENG ✅ · Full Engine suite ✅ (**engine 216 · server 13 · client 50**, typecheck สะอาด)
- Simulation 3 & 10 คน ✅ (`fullGame.test.ts` + `tenPlayerSoak.ts` — 0 crash/hang, card 32/32 · skill 25/25)
- `/code-review` (high effort) รันแล้ว — 5 finding (ไม่มี Critical/Major) แก้ครบ

**กฎที่เปลี่ยน/ทำให้ชัดในรอบนี้ (อัปเดตตาม gate):**

1. **เฟสจั่ว = ปุ่มกดเอง** ผ่าน `drawCard` decision; หมดเวลา Server จั่วให้ (default answer); จั่วครั้งเดียวต่อเทิร์น (กันจั่วซ้ำ). จิวยี่ "ปรีชาเจียงตง" = modifier +1 บังคับ รวมในธุรกรรมจั่วเดียว (จั่ว 3) พร้อม banner; เตียวเลี้ยว/เคาทู = ถามก่อนจั่ว.
2. **ลิโป้ "คู่ควรไร้เทียมทาน"** — เมื่อต้องลง "หลบ" 2 ใบ (หรือ "สังหาร" 2 ใบในดวล) ถามเป็น **ชุดเดียว all-or-nothing**: ลงครบ = รอด/ชนะรอบ, ลงไม่ครบ = โดน/แพ้ แต่**ไม่เสียการ์ด** (ไม่มีการทิ้งการ์ดทิ้งเปล่า).
3. **โจโฉ/สุมาอี้/กุยแก/แฮหัวตุ้น (OnDamaged/OnHPLost)** และ **คุ้มกันราชา/ปลุกใจนักรบ (OnNeedDodge/OnNeedSha)** — trigger/prompt เฉพาะผู้เล่นที่เป็นเป้าจริง (ผู้ถูกทำ damage / ผู้ต้องตอบโต้) เท่านั้น.
4. **อสนีบาตเวียนค่าย (สายฟ้า)** — `targetRule: "self"` วางที่ตัวเองเสมอ (client ไม่ต้องเลือกเป้า); ตัดสินโพดำ 2–9 = 3 ดาเมจ, ไม่เข้าเงื่อนไข/ถูกไร้ช่องโหว่กัน = ส่งต่อคนถัดไป (ข้ามผู้ตาย).
5. **Game Log** เก็บเป็น structured (`eventType` + ids, ไม่เก็บไทยเป็น identity); client resolve เป็นไทย; entry แบบ private (เช่น pickGeneral) เห็นเฉพาะเจ้าของ.
...
เมื่อผู้เล่นหมด Grace Period ระหว่าง Active Match:

- Token เก่าถูก revoke
- ผู้เล่นกลับหน้า Lobby หลักเมื่อเปิด Client อีกครั้ง
- ห้าม recovery ผ่าน Session เก่า
- ตัวละครใน Match ถือว่าเสียชีวิต
- เปิดเผย Role ของผู้เล่นคนนั้น
- ทิ้ง Hand, Equipment และ Delayed Tricks ตาม Death Flow
- การเสียชีวิตจาก Disconnect ไม่มีผู้สังหาร
- ไม่ให้รางวัลสังหารและไม่ใช้บทลงโทษจากการสังหาร
- ตรวจ Victory Condition หลัง Death Flow
- หากผู้เล่นที่หมด Grace Period เป็นเจ้าเมือง:
  - จบ Match ทันที
  - Result เป็น `no_winner`
  - ห้ามมอบชัยชนะให้กบฏ ไส้ศึก หรือฝ่ายอื่น
- หากห้องเดิมกลับเป็น Lobby และยังรับคน ผู้เล่นสามารถ Join ใหม่ด้วย Flow ปกติได้

การกด `ออกจากเกม` โดยตั้งใจระหว่าง Active Match ใช้ผลเดียวกันทันที โดยไม่ต้องรอ Grace Period
...
### 7.2 Role Reveal ก่อนเลือก General

Flow ที่ตกลงแล้ว:

1. สุ่ม Seat และ Role สำหรับ Match ใหม่
2. Role ไม่ผูกกับ Seat 1 เจ้าเมืองสามารถอยู่ Seat ใดก็ได้
3. เปิด Role Reveal Screen ชั่วครู่
4. ผู้เล่นแต่ละคนเห็นเฉพาะ Role ของตนเอง
5. ตัวตนของเจ้าเมืองเป็นข้อมูล Public ตามกฎ
6. เมื่อหมดเวลาหรือผู้เล่นกดยืนยัน จึงเข้าสู่ General Selection

Hidden Information:

- Client ห้ามได้รับ Role ของผู้เล่นอื่นที่ยังไม่ถึงเวลาสรุปผล
- ห้ามส่ง Role ทุกคนแล้วซ่อนด้วย CSS
- Rejoin ระหว่าง Role Reveal ต้องกลับมาที่ Screen และ Deadline เดิม

### 7.3 Character Selection แบบเลือกตามลำดับและเปิดเผยสองช่วง

ลำดับ:

1. เจ้าเมืองเลือกก่อน
2. Dialog ตัวเลือก General แสดงเฉพาะ Client ของผู้ที่ถึงคิวเลือก
3. ผู้เล่นที่ยังไม่ถึงคิวเห็น Waiting State เท่านั้น และไม่ได้รับ Candidate General IDs
4. เมื่อเจ้าเมืองยืนยัน:
   - เปิดเผย General ของเจ้าเมืองต่อทุกคนทันที
   - เริ่มคิวผู้เล่นคนถัดไปตามเข็มนาฬิกาจาก Seat ของเจ้าเมือง
5. ผู้เล่นที่ไม่ใช่เจ้าเมืองเลือกแบบส่วนตัวทีละคน
6. ระหว่างนั้นคนอื่นเห็นเพียงสถานะ `กำลังเลือก` หรือ `เลือกแล้ว`
7. เมื่อผู้เล่นที่ไม่ใช่เจ้าเมืองทุกคนเลือกครบ:
   - เปิดเผย General ของทุกคนพร้อมกัน
   - เริ่ม Match โดยให้เจ้าเมืองเป็นผู้เล่นเทิร์นแรก ไม่ว่าเจ้าเมืองจะอยู่ Seat ใด

หน้าจอผู้ที่ถึงคิวต้องมี:

- Role ของตน
- Timer
- Candidate Generals ของตนเท่านั้น
- ชื่อ General
- Faction
- HP
...
- หากหมดเวลา ให้ Server ใช้ Seeded RNG สุ่มจาก Candidate ที่ยังถูกต้อง
- การสุ่มต้อง deterministic จาก Match Seed
- ผู้เล่นอื่นห้ามเห็น Candidate ที่ไม่ได้ถูกเลือก

Server ต้อง:

- Validate choice
- ป้องกัน General ซ้ำ
- ไม่ผูกเจ้าเมืองกับ Seat 1
- ล้าง Private Options หลังเลือก
- Restore Current Selector, Candidate Set และ Deadline เมื่อ Rejoin
...
### 8.2 การสุ่ม Seat และ Role เมื่อเริ่ม Match

ทุก Match รวมทั้ง Match แรกและ Rematch ต้อง:

- ใช้ Seeded RNG สุ่มผู้เล่นจาก Room ไปยัง Match Seats
- สุ่ม Role โดยไม่ผูกกับ Seat Index
- ห้ามกำหนดให้ Seat 1 เป็นเจ้าเมืองโดยอัตโนมัติ
- เจ้าเมืองสามารถอยู่ Seat ใดก็ได้
- เจ้าเมืองเป็นผู้เริ่มเทิร์นแรก โดยตั้ง `currentSeat` ไปยัง Seat ของเจ้าเมือง
- General Selection เริ่มจากเจ้าเมือง แล้ววนตามเข็มนาฬิกาจาก Seat ของเจ้าเมือง
- Seat คงเดิมตลอด Match และห้ามสุ่มใหม่เมื่อ Reconnect
- Lobby order สามารถคง Join Order ได้ แต่ต้องแยกจาก Match Seat อย่างชัดเจน
...
### 8.4 Result Screen และสถิติหลังเกม

Result Screen ต้องเปิดเผย:

- Role ของผู้เล่นทุกคน
- General ของผู้เล่นทุกคน
- ฝ่ายที่ชนะ
- รายชื่อผู้เล่นที่ชนะ
- สาเหตุจบเกม
- จำนวน Turn
- ระยะเวลา
- ลำดับการเสียชีวิต
...
- ผู้เล่นที่สังหารมากที่สุด
- ผู้เล่นที่ได้รับความเสียหายรวมมากที่สุด

ข้อกำหนดสถิติ:

- Server คำนวณจาก Structured Game Events
- กรณีคะแนนเท่ากันให้แสดงผู้เล่นทุกคนที่เสมอกัน
- การเสียชีวิตจาก Leave/Grace Expiry ไม่มีผู้สังหารและไม่เพิ่ม Kill Count
- `Damage Taken` นับเฉพาะ Damage Event
- การเสีย HP จากต้นทุนสกิล เช่น โบยกายลวงศึก ไม่รวมใน Damage Taken
- ผู้เล่นที่ตายไปก่อนยังมีสิทธิ์ติดอันดับสถิติ
- หากเจ้าเมืองออกหรือหมด Grace Period:
  - แสดง `เกมยุติ — ไม่มีผู้ชนะ`
  - ยังแสดง Role และสถิติของ Match ได้

ข้อมูลที่แนะนำให้เก็บเพิ่มเพื่อรองรับอนาคต:
...
ก่อนสร้าง Match ใหม่:

- สุ่ม Match Seat ใหม่ทุกครั้งด้วย Seeded RNG
- สุ่ม Role ใหม่โดยไม่ผูกกับ Seat
- ห้ามใช้ Seat เดิมเพียงเพราะเป็น Rematch
- Lobby Join Order ไม่ถือเป็น Match Seat
...
## 19. Requirement Traceability — รวม Requirement ที่เคยแจ้งทั้งหมด

ส่วนนี้ใช้ตรวจว่าความต้องการเดิมทุกข้อถูกผูกเข้ากับงาน Implementation แล้ว  
สถานะในตารางนี้เป็นสถานะ Requirement ไม่ใช่สถานะว่าโค้ดเสร็จ

| ID | Requirement | Expected Behavior | Phase / Work Item |
|---|---|---|---|
| REQ-001 | Game History ใช้ชื่อจริง | Log แสดงชื่อผู้เล่น การ์ด สกิล และนายพลจาก Stable ID ไม่ใช้ `Player 1` หรือข้อความกำกวม | Phase 1 `ENG-009`, Phase 7 Game History |
| REQ-002 | ดูสกิลผู้เล่นอื่น | กด Player Panel เพื่อเปิด General Detail และเห็นเฉพาะสกิล Public พร้อมสถานะ | Phase 3, Phase 7 Player Detail |
| REQ-003 | Rejoin แล้วกลับมาแต่กดไม่ได้ | Rejoin ต้องคืน Authoritative State, Pending Decision, Deadline และ Legal Actions พร้อมล้าง Local Selection เก่า | Phase 2 Automatic Rejoin |
| REQ-004 | การ์ดล่าสุด Hover ไม่ได้ | Desktop Hover และ Mobile Tap ต้องเปิด Card Preview ได้ โดย Overlay ไม่บล็อก Pointer Event | Phase 7 Central Card Zone |
| REQ-005 | การจั่ว | ผู้เล่นกดจั่วเอง; หมดเวลาจั่วอัตโนมัติ; Optional Draw Skill ถามก่อน; Mandatory Modifier ทำพร้อมการจั่วและแจ้งชื่อสกิล | Phase 1 `ENG-004` |
| REQ-006 | เข้าห้องด้วยการสลับชื่อ | Identity ต้องใช้ Session Token/Player ID ห้ามใช้ Display Name ยึด Seat | Phase 2 Identity |
| REQ-007 | ออกจากห้องแล้วชื่อยังอยู่ | `leave-room` ต้องลบสมาชิกทันที ต่างจาก `disconnect` ที่อยู่ระหว่าง Grace Period | Phase 2 Join/Leave |
| REQ-008 | เห็น Role ก่อนเลือกนายพล | แต่ละคนเห็น Role ของตนชั่วครู่ก่อนเริ่ม General Selection | Phase 3 Role Reveal |
| REQ-009 | ซ่อน Role คนอื่นตอนเลือก | Server ห้ามส่ง Role และ Candidate Generals ของคนอื่น | Phase 3 Hidden Information |
| REQ-010 | ลำดับเลือกนายพล | เจ้าเมืองเลือกก่อนและเปิด General ทันที; คนอื่นเลือกส่วนตัวตามลำดับ; เปิดพร้อมกันเมื่อครบ | Phase 3 Character Selection |
| REQ-011 | เจ้าเมืองไม่อยู่ Seat 1 ตลอด | สุ่ม Match Seat และ Role แยกกันทุก Match รวม Match แรกและ Rematch | Phase 4 Match Start |
| REQ-012 | ทุกคนอื่นออกหมด | รอ Grace Period ก่อน; หากเล่นต่อไม่ได้ให้ Match เป็น `abandoned`, แจ้งผู้เล่นที่เหลือ และกลับหน้าแรก | Phase 2 All Players Gone |
| REQ-013 | ปุ่มออกจากห้อง | Lobby/Post-game ออกได้; Active Match ต้องยืนยันและใช้ Death-on-leave policy | Phase 2 Join/Leave |
| REQ-014 | Character Selection มี Interaction มากขึ้น | Timer, Random, Confirm, Waiting State, Current Selector และ Private Candidate Dialog | Phase 3, Phase 7 |
| REQ-015 | ขงเบ้งเรียงการ์ดเอง | Dialog แสดงการ์ดที่มีสิทธิ์เห็นและให้จัดบน/ใต้กอง พร้อม Validation ครบ | Phase 1 `ENG-007`, Phase 7 Dialog |
| REQ-016 | Highlight Turn และ Phase | Turn Panel กลางบน แสดงผู้เล่นปัจจุบัน เฟส ผู้ตอบ Turn Number และเวลา | Phase 5 View Model, Phase 7 Turn Panel |
| REQ-017 | อสนีบาตเวียนค่าย | เป็น Delayed Trick ติดเป้าหมาย ตัดสิน ♠ 2–9 เสีย 3 HP มิฉะนั้นส่งต่อ | Phase 1 `ENG-005` |
| REQ-018 | Death Dialog | ผู้ตายเห็น Dialog, ดูเกมต่อหรือออกได้; Seat เดิมไม่ขยับ | Phase 1 Death Flow, Phase 7 Dialog |
| REQ-019 | ตรวจ Seat ตอนเข้าห้อง | Server เป็น Source of Truth ของ Seat; Client แสดง Relative Seat จาก Viewer | Phase 2 Seat consistency, Phase 7 Board |
| REQ-020 | ผู้เล่นต้องรู้ว่าตนอยู่ตรงไหน | Local Player อยู่ล่างกลางเสมอ พร้อม Player/General/Role ของตนชัดเจน | Phase 7 Board |
| REQ-021 | Mobile | Lobby Portrait ได้; Character Selection และ Match เป็น Landscape-first พร้อม Rotate Overlay | Phase 8 |
| REQ-022 | เลือกการ์ดทิ้งเกินจำนวน | UI ป้องกัน และ Server/Engine Validate `min/max/exact` ซ้ำ | Phase 1 `ENG-002`, Phase 7 Hand |
| REQ-023 | โจโฉ Trigger ผิด | สกิล “พลิกภัยเป็นกล” ทำงานเฉพาะเมื่อโจโฉเจ้าของสกิลโดน Damage เอง | Phase 1 `ENG-006` |
| REQ-024 | Lobby ใช้ Theme เดียวกับเกม | Lobby ใช้บรรยากาศโต๊ะบัญชาการสามก๊ก แต่ทำหลัง Functional UI | Phase 7 Lobby, Phase 9 Theme |
| REQ-025 | Center Table | กลางโต๊ะมี Draw Pile, Latest/Resolving Card และ Discard Pile เท่านั้น | Phase 7 Central Card Zone |
| REQ-026 | Delayed Trick ติดผู้เล่น | การ์ดอุบายล่าช้าแสดงบน Player เป้าหมาย ไม่วางค้างกลางโต๊ะ | Phase 5 PlayerView, Phase 7 Board |
| REQ-027 | Local Player ล่างกลาง | ใช้ Relative Seat Formula โดยไม่แก้ Seat จริงของ Server | Phase 7 Board |
| REQ-028 | กลางบนสงวน Turn Panel | ไม่ใช้เป็น Seat ผู้เล่น แสดง Turn/Phase/Timer แทน | Phase 7 Board |
| REQ-029 | รองรับ 10 คน | 3–5 Medium, 6–8 Compact, 9–10 Head Portrait Layout | Phase 7, Phase 8 |
| REQ-030 | End Turn/Timeout ชัด | แสดง Action Prompt, ปุ่มยืนยัน/จบเฟส และ Deadline จาก Server | Phase 5, Phase 7 |
| REQ-031 | มือและอุปกรณ์ของตน | Local Area ใหญ่ แสดง Hand, Equipment, Skill และ Selection State | Phase 7 |
| REQ-032 | อุปกรณ์ผู้เล่นอื่น | Compact Icons บน Player Panel และดูรายละเอียดผ่าน Dialog | Phase 7 |
| REQ-033 | Player Detail Modal | แสดง General, Skills, Equipment, Delayed Tricks, Hand Count, Distance และ Range Result | Phase 7 |
| REQ-034 | Mobile ใช้โครงสร้างคล้าย Desktop | รักษาโครงสร้างโต๊ะ แต่เปลี่ยนเป็น Compact, Tap-first และ Full-screen Dialog ไม่ใช่แค่ย่อ Desktop | Phase 8 |
| REQ-035 | Automatic Rejoin | ไม่มี Manual Rejoin; Default Grace 45 วินาที; Server ตัดสินสิทธิ์ | Phase 2 |
| REQ-036 | Grace Expiry | ผู้เล่นถือว่าเสียชีวิต เปิด Role ไม่มี Killer/Reward; เจ้าเมืองออกแล้ว `no_winner` | Phase 2 |
| REQ-037 | Result Screen | เปิด Role ทุกคน แสดง Winner/No Winner, Most Kills และ Most Damage Taken | Phase 4 |
| REQ-038 | เล่นต่อหลังจบเกม | ใช้ Socket เดิม กลับ Lobby เดิม Ready ใหม่ และสร้าง Match ID/State ใหม่ | Phase 4 |
| REQ-039 | สุ่ม Seat ทุก Match | Match แรกและ Rematch สุ่มใหม่ด้วย Seeded RNG | Phase 4 |
| REQ-040 | Artwork ยังไม่ใส่ | เตรียม Asset Resolver และ Placeholder สำหรับ Card/General full/portrait/head | Phase 9 |
| REQ-041 | ชื่อไทยการ์ด/อุปกรณ์/สกิล | ใช้ Thai Naming Catalog โดยไม่เปลี่ยน Internal ID หรือ Mechanic | Phase 6, ภาคผนวก E |
| REQ-042 | UI Theme | Dark wood, bronze, parchment, Chinese war-table; ไม่เอา Shop/Mail/Chat/Sound จากภาพอ้างอิง | Phase 9 |

### 19.1 กฎการอัปเดตสถานะ

- Requirement ที่ “ตัดสินแล้ว” ไม่เท่ากับ Implementation เสร็จ
- TODO ใช้ `[x]` ได้ต่อเมื่อ:
  - โค้ดถูกแก้
  - Test ผ่าน
  - Typecheck/Lint ผ่าน
  - GPT Review ไม่มี Critical/Major issue
- รายการที่ตรวจแล้วว่าไม่ใช่ Bug และไม่ต้องทำอะไร เช่น “ส่งไปส่อง” จะไม่สร้าง Work Item ใน TODO
...
```
1. สุ่ม Seat ของผู้เล่นทุกคนด้วย seeded RNG
2. สุ่มแจกบทบาทโดยไม่ผูกกับ Seat 1
3. แสดง Role ของตนเองให้ผู้เล่นแต่ละคนชั่วครู่
4. เจ้าเมืองเปิดเผยตัวและเลือกนายพลก่อน (สุ่มให้เลือก 5 ตัว)
5. เปิดเผยนายพลของเจ้าเมืองทันที
6. ผู้เล่นอื่นเลือกนายพลแบบส่วนตัวตามลำดับที่นั่งจากเจ้าเมือง (สุ่มให้เลือกคนละ 3 ตัว)
7. เมื่อผู้เล่นอื่นเลือกครบ เปิดเผยนายพลของทุกคนพร้อมกัน
8. พลังชีวิตสูงสุด:
   - เจ้าเมือง = ค่านายพล + 1
   - คนอื่น    = ค่านายพล
9. ทุกคนจั่ว 4 ใบ
10. เจ้าเมืองเริ่มเทิร์นแรกจาก Seat ที่สุ่มได้ → วนตามลำดับที่นั่ง
```
